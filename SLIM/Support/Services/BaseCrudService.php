...
namespace SLIM\Support\Services;
...
use SLIM\Support\Repositories\BaseRepositoryInterface;
...
abstract class BaseCrudService implements BaseCrudServiceInterface
{
     ...
     */
    public function getAllPaginated(array $search = [], int $pageSize = 15, array $withRelations = [],array $withCountRelations = []): LengthAwarePaginator
    {
        return $this->repository->getAllPaginated($search, request()->get('page_size', $pageSize),$withRelations,$withCountRelations);
    }
     ...
     * @param array $attributes
     * @param array $data
     * @return Model|null public function datatable(?array $filters = []): Builder
    {
        dd($filters);
        return $this->applyFilters(searchParams: $filters)->withCount('subscribes');
    }
     * @throws ServiceException
     */
